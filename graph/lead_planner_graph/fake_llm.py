"""A deterministic LLM that satisfies the :class:`LLM` Protocol.

Used by the test suite and by ``run.py --demo`` so the whole graph — including
interrupts and the delegate/review loop — can be exercised without a running LM
Studio server or the little-coder CLI. It picks a reply by detecting the current
phase from the system prompt (``# Current phase: <id>``).
"""
